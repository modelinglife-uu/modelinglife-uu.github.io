...
Plant roots grow thanks to the presence of stem cells housed in a niche at the root tip. 
A subset of these cells express *WOX5*, maintaining the surrounding cells undifferentiated. 
...
auxin promotes its expression, while others found it is repressed (Figure 1). Notably, auxin does
this opposite reglation through different ARFs (Figure 2) posing the possibility that 
...
attractorswe studied describe the columella, In this model
WOX5 is regulated positively by ARF5, and negatively by ARF10 and also by
...
week. Here you will explore whether this is relevant to understanding the 
auxin–WOX5 paradox. For this you will use an *in silico* root model in which 
...
(Figure 3, right). The tissue is modelled as a grid, where the x position defines 
the radial direction and the y the longitudinal position (0 being the cells at the bottom). 
...
1.- `Rootfunctions.py` – File storing the functions used in `Root-model-Auxin.py`. For this practical you 
only need to modify `rootNetwork`. Yet, have a look at all of them and try to understand what they do. 
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  > of a given cell (`findNeighbors`), and then calculate how much auxin should they 
  > exchange based on how much auxin they have (`auxinGrid`) and what are their cell 
  ...
  > of the network (e.g. WOX5grid, CKgrid, etc.) into a `cellState` vector. `cellState` is 
...
How does auxin treatment affect gene expression in the root? Does the model output correspond to the transcriptional reporter of WOX5 shown in Figure 1? Do you see activation or repression as
reported experimentally?(You can
...
hypothesis and aim to recover such behaviours (Figure 1). To account for quantitative effects 
of auxin on gene regulation, let' implement the following two hypotheses based on the
literature:

![Testing two hypothesis: <span style="color: magenta;">Auxin represses MGP expression.</span> <span style="color: #32CD32;">Extremely high auxin levels (unrealistic in vivo) repress WOX5 expression.</span>](images/0-auxin-hypothesis.png){#vegmini1 width=60%}
...
This matches experimental microscopy. 

- Why do different parts of the root respond differently to auxin? 
- Why does auxin regulation of MGP do? (Tip: look at the network...)
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  >
  > - because auxin is distributed in a gradient so the cells at the tip will reach the repressive levels before the distal cells. 
...
tip. Reproduce this *in silico* by saving the average WOX5 levels of
...
Then plot this average WOX5 expression levels for `auxinTreatment`= 0, 
10 and 750.
...
Now, let's compare WOX5 regulation across tissues. Repeat the RT-PCR *in silico* analysis but
include the cells in the QC position, and endodermis cells. Tip: cell types are stored in cellGrid.
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  > **Answer** The repression of WOX5 expression is much more clear if we sample individual QC cells, 
  while sampling only endodermal cells only shows activation. In a way both results are correct, and reveal how dependent on spatial context the effects of auxin are. Moreover it shows how difficult it is to draw conclusions from this sort of experiments when taking a sample that contains a mixture of cell types. It is actually very diffiult to only sample individual cell types, so sampling the whole tissue is the best option, but we have to be aware that tissues are composed of many cell types, and each may respond differently!
   ![](images/0-insilico-RT-PCR_plots copy.png){#vegmini1 width=100%}
::: 
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  > **Answer** It is clearly dependent on levels of auxin and the cell type. Which makes sense given that this hormone is implicated in so many different processes in plants. The underlying gene regulatory networks, and cell differentiation, enables a diversity of responses.   
...
fun), the original experiments used different auxin *analogs* which are transported 
very differently between cells: NAA (that is transported independently of PINs), 
IAA (that is PIN-transported), and NPA (that blocks PINs, increasing auxin in the 
