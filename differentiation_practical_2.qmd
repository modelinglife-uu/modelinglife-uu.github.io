...
:::{.content-visible when-meta="params.show_answers"}
  ...
  > This change can be implemented by putting both `auxinTransport` and the network update inside 
  > conditionals with different values (`if step % x == 0:`) where x is larger for auxin transport than 
  > for gene regulation. 
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  ![Figure 3. Ingredients of the in silico root model. 1) gene regulatory network, 2) multi-cellular tissue layout with polar auxin transport in the root tip](images/0-model-incomplete.png){#vegmini1 width=100%}
...
![Figure 4. <span style="color: magenta;">Auxin represses MGP expression.</span> <span style="color: #32CD32;">Extremely high auxin levels (unrealistic in vivo) repress WOX5 expression.</span>](images/0-auxin-hypothesis.png){#vegmini1 width=60%}
...
What is the root tip response to auxin treatments? Considering that
...
Now, let's compare WOX5 regulation across tissues. Repeat the qPCR *in silico* analysis but
include the cells in the QC position, and endodermis cells. Tip: cell types ar stored in cellGrid.
...
:::{.content-visible when-meta="params.show_answers"}
  ...
  while sampling only endodermal cells only shows activation. In a way both results are correct, and reveal
  how dependent on spatial context the effects of auxin are. Moreover it shows how difficult it is to draw conclusions from this sort of experiments, as it is actually very diffiult to only sample individual cell types. Sampling the whole tissue is the best option, but we have to be aware that tissues are composed of many cell types, and each may respond differently!
...
All models are wrong, but some are useful. The model we studied today offered 
mechanistic insight into the paradoxical auxin responses. Yet, to make things more fun, 
the original experiments used different auxin analogs: NAA (that is transported
independently of PINs), IAA (that is PIN-transported), and NPA (that blocks PINs,
increasing auxin in the tip). How does this information change your
